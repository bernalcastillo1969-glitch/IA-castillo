...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-[#12121f] text-[#e3e0f3] font-sans selection:bg-primary/30 overflow-x-hidden scroll-smooth transition-colors duration-500">
      ...
      {/* ── Navbar (CORREGIDA ALINEACIÓN MÓVIL) ── */}
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center justify-between px-6 md:px-8 py-3 bg-[#343342]/70 backdrop-blur-xl border border-white/10 rounded-full w-[92%] max-w-5xl shadow-[0_10px_30px_rgba(0,0,0,0.3)] mx-auto">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          <a href="/chat" className="hidden sm:inline-block px-6 py-2 rounded-full text-sm font-bold bg-gradient-to-r from-[#d0bcff] to-[#a078ff] text-[#3c0091] hover:scale-105 active:scale-95 transition-all shadow-lg">
            Comenzar
          </a>
          {/* Mobile Toggle Mejorado */}
          <button 
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)} 
            className="md:hidden p-2 rounded-full bg-white/5 border border-white/10 text-white active:scale-90 transition-all flex items-center justify-center"
          >
      ...
      {/* ── Mobile Menu (CORREGIDA ALINEACIÓN CHUECA) ── */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="fixed top-24 left-1/2 -translate-x-1/2 w-[92%] z-[45] md:hidden bg-[#1e1e2c]/98 backdrop-blur-2xl rounded-3xl border border-white/10 p-8 shadow-2xl flex flex-col items-center"
          >
            <div className="w-full flex flex-col gap-5 text-center">
              {navItems.map((item) => (
                <a key={item.id} href={item.href} 
                  onClick={() => { setActiveItem(item.id); setMobileMenuOpen(false); }}
                  className={`text-xl font-bold py-2 transition-all ${activeItem === item.id ? 'text-[#4cd7f6]' : 'text-[#cbc3d7]'}`}
                >
                  {item.label}
                </a>
              ))}
              <div className="h-px bg-white/5 my-2 w-full" />
              <a href="/chat" className="w-full py-4 rounded-2xl bg-gradient-to-br from-[#d0bcff] to-[#a078ff] text-[#3c0091] font-black text-xl shadow-xl active:scale-95 transition-transform">
                Comenzar Chat
              </a>
            </div>
          </motion.div>
      ...
      <main>
        ...
        <section className="relative z-10 py-32 px-6">
          <div className="max-w-7xl mx-auto text-center mb-20"><h2 className="text-5xl font-bold mb-6 text-white">Todo en <span className="text-[#d0bcff]">Uno</span></h2><p className="text-[#cbc3d7] text-xl max-w-2xl mx-auto font-medium leading-relaxed">Un ecosistema completo de herramientas diseñadas para integrarse sin fricciones.</p></div>
        ...
        {/* ── FEATURES ── */}
        <section id="features" className="relative z-10 py-32 px-6 text-left italic"><div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center"><h2 className="text-4xl md:text-5xl font-bold text-white">Interacción en <span className="text-[#4cd7f6]">Tiempo Real</span></h2><ChatMockup title="Chat de IA Castillo v2.4" /></div></section>
        ...
        <section id="security" className="relative z-10 py-32 px-6">
          <div className="max-w-7xl mx-auto">
            <div className="bg-gradient-to-br from-[#1e1e2c] to-[#04040f] rounded-[3rem] p-12 md:p-24 border border-[#4edea3]/20 text-left relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
                <div className="flex-1"><h2 className="text-5xl md:text-6xl font-bold mb-10 text-white">Protección <br/><span className="text-[#4edea3]">Empresarial</span></h2><div className="flex gap-6"><Lock className="text-[#4edea3]" /><div><h4 className="text-2xl font-bold text-white">Supabase RLS</h4><p className="text-[#cbc3d7] font-medium leading-relaxed">Seguridad de nivel bancario e integridad de datos garantizada.</p></div></div></div><ShieldCheck size={180} className="text-[#4edea3] opacity-30 shadow-2xl" />
            </div>
          </div>
        </section>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="bg-[#343342]/40 backdrop-blur-xl rounded-[4rem] p-12 md:p-20 border border-white/5 relative overflow-hidden text-left shadow-2xl">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 relative z-10 text-left">
                ...
                <div className="lg:col-span-7 grid grid-cols-1 md:grid-cols-2 gap-8 italic text-left">
                  ...
                  <div><h4 className="font-bold text-white mb-2">Gestión y Despliegue</h4><p className="text-[#cbc3d7] text-sm leading-relaxed text-left">Manejo eficiente de datos y producción en Vercel con Gemini.</p></div>
                </div>
              </div>
            </motion.div>
          </div>
